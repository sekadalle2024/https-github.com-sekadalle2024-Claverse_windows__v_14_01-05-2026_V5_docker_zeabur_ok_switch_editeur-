function addCriteriaTablesToChatTables() {
  ...
  // Fonction corrigée pour extraire TOUTES les tables des divs contenant des tables Frap (HTML brut)
  function getCriteriaTablesWithKeyword() {
    const tablesHTML = [];
    const processedDivs = new Set(); // Pour éviter de traiter plusieurs fois la même div
    
    console.log('🔍 Recherche des tables de type Frap et collection de toutes les tables de leurs divs...');
    ...
    allTables.forEach((table, index) => {
      ...
      if (hasRubrique && hasDescription) {
        ...
        // Rechercher le mot-clé "Frap" dans TOUTES les cellules de la table
        ...
        // Si le mot-clé "Frap" est trouvé, collecter TOUTES les tables de cette div
        if (foundFrapKeyword) {
          ...
          if (parentDiv && !processedDivs.has(parentDiv)) {
            ...
            // Récupérer TOUTES les tables de cette div
            const allTablesInDiv = parentDiv.querySelectorAll('table.min-w-full.border.border-gray-200.dark\\:border-gray-700.rounded-lg');
            
            console.log(`🎯 Table Frap trouvée ! Collecte de TOUTES les ${allTablesInDiv.length} table(s) de cette div`);
            
            allTablesInDiv.forEach((divTable, divTableIndex) => {
              tablesHTML.push(divTable.outerHTML);
              console.log(`✅ Ajout table HTML ${divTableIndex + 1}/${allTablesInDiv.length} de la div Frap`);
            });
          }
        } else {
    ...
    console.log(`📊 Résultat final: ${tablesHTML.length} table(s) HTML collectée(s) depuis ${processedDivs.size} div(s) contenant des tables Frap`);
  ...
  chatTables.forEach((targetTable, index) => {
    ...
    if (caseType === 5 && !hasFlowiseContainer) {
      ...
      const processFlowiseRequest = async () => {
        try {
          // Obtenir TOUTES les tables des divs contenant des tables Frap (HTML brut)
          ...
          console.log(`🔍 ${tablesCount} table(s) HTML collectée(s) depuis les divs contenant des tables Frap`);
